...
const CommunityAxiosApi = {
  ...
  // 게시글 조회 (카테고리 별)
  getCommunityListByCategory: async (id, page, size) => {
    return await axios.get(
      BACKEND_DOMAIN +
        `/api/community/list/page/category?categoryId=${id}&page=${page}&size=${size}`
    );
  },
  // 카테고리 조회
  cateList: async () => {
  ...
  // 카테고리에 따른 페이지 수 조회
  getCommunityTotalPagesByCategory: async (id, size) => {
    return await axios.get(
      BACKEND_DOMAIN + `/api/community/count/${id}?page=0&size=${size}`
    );
  },
  // 게시글 등록
